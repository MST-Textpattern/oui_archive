h1. oui_player_archive

h2. Introduction

"oui_player_archive":https://github.com/NicolasGraph/oui_player_archive is a "oui_player":https://github.com/NicolasGraph/oui_player v2+ extension to easily embed "Archive":https://archive.org/ provided medias in "Textpattern":https://textpattern.com/ powered websites.
...
oui_player_archive's minimum requirements:
...
# "Download":https://github.com/NicolasGraph/oui_player_archive/releases the compiled plugin file or the source to compile a customized file.
...
# Install oui_player_archive:
@$ composer require oui/oui_player_archive@
...
bc(language-markup xml). $ composer update oui/oui_player_archive
...
bc(language-markup xml). $ composer remove oui/oui_player_archive

h2. Tags and parameters

h3. oui_archive vs oui_player

While the @<txp:oui_player />@ tag is able to embed any media from any extension-related provider, the @<txp:oui_archive />@ tag is a shortcut to embed Archive medias only. It is strictly equivalent to @<txp:oui_player provider="archive" />@.
...
h3. oui_if_archive vs oui_if_player

p. While the @<txp:oui_if_player>…</txp:oui_if_player>@ tag is able to check an URL against any URL from any extension-related provider, the @<txp:oui_if_archive>…</txp:oui_if_archive>@ tag is a shortcut to check an URL against Archive related URL schemes only. It is strictly equivalent to @<txp:oui_if_player provider="archive">…</txp:oui_if_player>@.
...
"oui_player":https://github.com/NicolasGraph/oui_player preference related field value: @https://archive.org/details/CC_1916_07_10_TheVagabond@.

h3. Embed a player by defining its provider and a media ID through attributes

bc(language-markup xml). <txp:oui_player provider="archive" play="CC_1916_07_10_TheVagabond" />

…or:

bc(language-markup xml). <txp:oui_archive play="CC_1916_07_10_TheVagabond" />

h3. Conditonally embed a player using attributes

bc(language-markup xml).. <txp:oui_if_player provider="archive" play="https://archive.org/embed/CC_1916_07_10_TheVagabond">
    <txp:oui_player />
</txp:oui_if_player>

p. …or:

bc(language-markup xml).. <txp:oui_if_archive play="https://archive.org/embed/CC_1916_07_10_TheVagabond">
    <txp:oui_archive />
</txp:oui_if_archive>
...
oui_player_archive version 1.0.0, Copyright (C) 2018 Nicolas Morand
...
* *oui_player_archive v1.0.0-beta* (2018-06-24)
